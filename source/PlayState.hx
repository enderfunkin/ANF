...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		...
		['Damnit', 0.6], //From 50% to 59%
		['Nice', 0.69], //From 60% to 68%
		['Cool', 0.7], //69%
		['Good', 0.8], //From 70% to 79%
		['Nice', 0.9], //From 80% to 89%
		['Great', 1], //From 90% to 99%
		['FC', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	var dialogue:Array<String> = ['AMOGUS', 'U SUS'];
	...
	override public function create()
	{
		...
		for (file in FileSystem.readDirectory("assets/scripts/songscripts"))
		{
			if(file.endsWith('.lua') && !filesPushed.contains(file))
			{
				luaArray.push(new FunkinLua("assets/scripts/songscripts/"+file));
				filesPushed.push(file);
			}
		}
	...
	override public function update(elapsed:Float)
	{
		...
		if(ratingName == '?') {
			...
			scoreTxt.text = 'Score: ' + songScore + ' | Combo Breaks: ' + songMisses + ' | Rating: ' + ratingName + ' | Accuracy: (' + Highscore.floorDecimal(ratingPercent * 10, 0) + ') Star' + ' - ' + ratingFC;//peeps wanted no integer rating
		}
